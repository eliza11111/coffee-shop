...
//!Инпуты для создание нового продукта
...
//для отображения данных в браузере
const sectionRead = document.getElementById("section__read");

//селекторы для редактирование
const inpEditDetails = document.getElementsByClassName(
  "window__edit_details"
)[0];
const inpEditPrice = document.getElementsByClassName("window__edit_price")[0];
const inpEditQuantity = document.getElementsByClassName(
  "window__edit_quantity"
)[0];
const inpEditSales = document.getElementsByClassName("window__edit_sales")[0];
const inpEditCategory = document.getElementsByClassName(
  "window__edit_category"
)[0];
...
async function render() {
  ...
  data.forEach((product) => {
    ...
    <div class="product__img" style="background-image:url(${
      product.urlImg
    })"></div>
    ...
    <span>Скидка:${product.sales}%</span>
    </div>
    <div>
    <span class="btn-like" onclick="toggleLike(${product.id})"> 
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"> 
              <path fill="${product.liked ? "#f05542" : "#000"}"
...
async function handleEdit(id) {
  ...
  inpEditSales.value = productToEdit.sales;
